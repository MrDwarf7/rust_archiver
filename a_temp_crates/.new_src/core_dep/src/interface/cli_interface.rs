pub mod cli_interface {
    use crate::interface::helpder_structs::helpder_structs::ValidArgs;

    use clap::{parser::RawValues, Arg, ArgAction, Command, ValueHint};
    use emojis;
    use std::{fmt, fmt::Debug, io, path::PathBuf};

    pub struct InterfaceParser {
        pub args: Vec<String>,
    }
    // InterfaceParser - Bsse constructor
    ...
    impl InterfaceParser {
        pub fn setup_runtime(&self) -> ValidArgs {
            ...
            let args_to_print = ArgsToPrint {
                src_path: matches.get_raw("src_path").unwrap(),
                archive_name: matches.get_raw("archive_name").unwrap(),
                archive_all_before: matches.get_raw("archive_all_before").unwrap(),
                non_interactive_mode: matches.get_raw("non_interactive_mode").unwrap().clone(),
            };
            args_to_print.debug_print();
            // println!("Args to print: {:#?}", args_to_print);

            let src_path = &matches
                .get_one::<PathBuf>("src_path")
                .expect("No source path provided")
                .to_path_buf();
            let archive_name = matches
                .get_one::<PathBuf>("archive_name")
                .expect("No archive name provided")
                .clone();
            let archive_all_before_str = matches
                .get_one::<String>("archive_all_before")
                .expect("No date provided");
            let archive_all_before =
                cli_helpers::parse_date(&archive_all_before_str).expect("Invalid date provided");

            self.printable_args();

            let valid_args_closure = || {
                let flags: Vec<String> = non_interactive_mode
                    .into_iter()
                    .map(|flag| flag.to_str().unwrap().to_owned())
                    .collect::<Vec<String>>();
